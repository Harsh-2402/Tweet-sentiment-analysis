...
app = Flask(__name__)
model = pickle.load(open('Portable_Model.pkl', 'rb'))
...
def predict():
    ...
    '''
    test = processAll(request.form['txt_tweet'])

    final_features = tfidf.transform([test])
    prediction = model.predict(final_features)
    cat_dict = {
        0: "Postive" +"  \U0001F60A",
        1: "Negative"+"  \U0001F61F"
    }
    output = cat_dict.get(prediction[0])
    tweet = request.form.values()

    return render_template('index1.html', prediction_text='{}'.format(output), tweet='{}'.format(tweet))
